...
formating depending on the type of _SPECIAL_BLOCK."
  ...
  ;; -- Reload firefox window on save
  ;; From https://github.com/fmdkdd/dotfiles/blob/2f9ea61d848d9a77376cc8a89b5d25889f210271/emacs/.emacs.d/elisp/utils.el#L150-L288
  (defvar-local fmdkdd/browser-window-list nil
    "List of the browser windows that will receive a F5 key event
when `fmdkdd/reload-browser-windows' is called. Elements of the
list are cons cells (ID . FOCUS) where ID is a X window id
obtained from running 'xdotool selectwindow' (a string
...
user through `fmdkdd/select-browser-windows'. This value is used
by `fmdkdd/reload-browser-windows' when the buffer-local variable
`fmdkdd/browser-window-list' is nil.")
...
windowactivate. Google Chrome and Chromium do not listen to the
key sent by xdotool when the window does not have focus (see
...
aforementioned variables for future calls. With a prefix
argument, it bypasses the variables and forces the selection of
the window. When the prefix argument is a number greater than 1,
multiple windows can be selected."
    (interactive "P")
...
event."
    (let ((args)            ; xdotool command to reload each window
          (wactivate))      ; must we switch the focus away from the
                            ; Emacs window?
      (setq args
            (seq-mapcat
             (lambda (w)
             ...
             window-list))
      ;; If one window needed the focus, we need to save the current
      ;; Emacs window and get focus back to it once we are done. We
      ;; leverage the WINDOW_STACK of xdotool to do that.
      (if wactivate
